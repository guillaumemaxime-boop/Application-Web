...
Vidéo optionnelle (auto-hébergée) sur fiches mobilier/expo et Studio. **SP1 (ADR-0021)** : transcodage **asynchrone** vers un mp4 web-ready + poster auto + métadonnées, porté par une entité `Video` à statut. **SP2 (ADR-0021)** : **streaming adaptatif HLS** (rendus multi-résolutions + manifeste, player hls.js/natif, fallback mp4). **SP3 (ADR-0021)** : **housekeeping** — nettoyage immédiat au remplacement/suppression + GC des orphelins (entités non référencées + fichiers `vid-*` sans entité) avec période de grâce. `VideoService` (distinct de `PhotoService`) : allowlist `.mp4`/`.webm`/`.vtt`, upload **streamé** (`Files.copy(InputStream)`), garde anti-path-traversal, noms **UUID immuables**.
...
**Housekeeping (SP3)** : deux niveaux. **(1) Nettoyage immédiat** — `deleteIfUnreferenced(id)` (garde `isReferenced` = `existsByVideoId` sur furniture/exhibition + clé `studio.video.id`) appelé au **remplacement** de la vidéo d'une fiche/Studio et à la **suppression** d'un propriétaire ; supprime l'ancienne vidéo (entité + fichiers) uniquement si plus aucun propriétaire ne la référence. **(2) GC batch** `gcOrphans(dryRun)` — recense (a) les entités `Video` non référencées et (b) les fichiers/dossiers `vid-*` du `uploadDir` sans entité connue, en excluant tout ce qui est protégé par la **période de grâce** (`UPLOADED`/`PROCESSING`, ou `READY`/fichier de moins de `app.video.gc-grace-hours` = 24 h par défaut) — protège les uploads frais non encore rattachés à une fiche sauvegardée. `dryRun=true` (défaut) **recense sans supprimer** (aperçu avant exécution) ; `dryRun=false` supprime. Le scan cible **exclusivement** les entrées préfixées `vid-` (photos `{uuid}.ext` jamais touchées). Suppression confinée au `uploadDir` (`startsWith` sur chemin normalisé, y compris garde défensive interne à `deleteDirRecursive` ; `Files.walk` sans suivi de symlink).
...
| POST | `/api/admin/videos/gc?dryRun` | JWT | GC des orphelins (`dryRun` **défaut `true`** = recense sans supprimer) → `{orphanVideos, orphanFiles, deleted}`. Exclut la période de grâce (24 h). Ne cible que les entrées `vid-*`. |
...
| DELETE | `/api/admin/videos/{id}` | JWT | Supprime fichiers (source/output/poster + `{id}-hls/`) + entité → 204/404. |
...
**CSP** : `media-src 'self'`. **Upload** : limite **200 Mo** (Spring + Nginx, à garder en phase). Pas de JS inline. **Livré** : SP1 (transcodage async) · SP2 (HLS adaptatif) · SP3 (housekeeping/GC). **Backlog** : déclenchement périodique/planifié du GC (aujourd'hui manuel via `POST /gc`).
