...
Vidéo optionnelle (auto-hébergée) sur fiches mobilier/expo et Studio. **SP1 (ADR-0021)** : transcodage **asynchrone** vers un mp4 web-ready + poster auto + métadonnées, porté par une entité `Video` à statut. **SP2 (ADR-0021)** : **streaming adaptatif HLS** (rendus multi-résolutions + manifeste, player hls.js/natif, fallback mp4). **SP3 (ADR-0021)** : GC des **fichiers disque `vid-*` sans entité** (rebut technique — voir ci-dessous). **SP4 (ADR-0021)** : **médiathèque vidéo** — modèle bibliothèque, liste/suppression explicite, picker de réutilisation. `VideoService` (distinct de `PhotoService`) : allowlist `.mp4`/`.webm`/`.vtt`, upload **streamé** (`Files.copy(InputStream)`), garde anti-path-traversal, noms **UUID immuables**.
...
**HLS (SP2)** : après le mp4, `transcode` génère en **best-effort** (échec ⇒ reste `READY`, fallback mp4) un HLS **multi-rendition TS** dérivé du mp4 normalisé, dans `{id}-hls/` (`master.m3u8` + `{0,1,2}.m3u8` + segments `.ts`), escalier **360/720/1080p** plafonné à la hauteur source. Colonne `hls_master_filename` (migration 036). Endpoint **batch** `POST /api/admin/videos/hls` (JWT) pour l'existant (idempotent). Exécution des process ffmpeg via **fichier temp** (`runToFile`, anti-blocage de pipe sur longues vidéos). `delete` retire récursivement `{id}-hls/`. Player : `<app-video-player>` choisit **HLS natif** (Safari/iOS), sinon **hls.js** (Chrome/FF/Edge), sinon **fallback mp4** ; dépendance `hls.js` (1ʳᵉ lib player tierce).

**Modèle bibliothèque (SP4)** : une entité `Video` est un **asset persistant, indépendant des fiches**. Elle vit jusqu'à suppression **explicite** depuis la médiathèque vidéo. Remplacer ou détacher une vidéo d'une fiche/Studio ne la supprime plus. Plusieurs fiches peuvent référencer la même vidéo. Les hooks `deleteIfUnreferenced` de SP3 (nettoyage immédiat au remplacement/suppression) sont **retirés**. `isReferenced` / `existsByVideoId` sont conservés pour l'affichage des usages (`usedBy`) et le garde-fou de suppression (409). **GC disque (SP3, simplifié)** : `gcOrphans(dryRun)` cible **uniquement** les fichiers/dossiers `vid-*` du `uploadDir` **sans entité connue** (rebut technique — restes d'échecs ou de suppressions). Le scan ne cible que les entrées préfixées `vid-` ; suppression confinée au `uploadDir` (`startsWith` normalisé ; `Files.walk` sans suivi de symlink). `dryRun=true` (défaut) = aperçu sans suppression. **Interface admin (SP4)** : page **Médiathèque vidéo** (`/admin/mediatheque-video`) — grille de cartes avec badge de statut, bloc **« Utilisée par »** (liens vers fiches mobilier/expo + Studio), upload central avec polling, suppression bloquée UI si `usedBy` non vide (filet serveur 409). Composant **`<app-video-picker>`** (modale) branché dans `<app-video-field>` — bouton **« Choisir une vidéo existante »** pour réutiliser une vidéo. **`<app-video-player>`** réagit au changement de source (`ngOnChanges`). Navigation admin : entrée **« Médiathèque vidéo »** ajoutée ; **« Médiathèque »** renommé **« Médiathèque photo »**.

| Méthode | Endpoint | Auth | Description |
|---------|----------|------|-------------|
| GET | `/api/videos/files/{*filename}` | public | Sert vidéo/sous-titres **et fichiers HLS imbriqués** (`{id}-hls/master.m3u8`, `.ts`). **HTTP Range** (`ResourceRegion`) : `206` si `Range`, `200` sinon, `416` hors borne ; `Accept-Ranges: bytes` ; cache `immutable`. Content-type `video/mp4`/`video/webm`/`text/vtt`/`application/vnd.apple.mpegurl`/`video/mp2t`. Garde anti-path-traversal (`startsWith(uploadPath)`) sur le chemin imbriqué. |
| GET | `/api/admin/videos` | JWT | Liste toutes les vidéos (tous statuts, tri `createdAt` décroissant) → `VideoSummary[]` (id, status, originalName, url, poster, hls, durée, dimensions, createdAt, `errorMessage` si FAILED, `usedBy`). |
...
| POST | `/api/admin/videos/hls` | JWT | Batch HLS sur les vidéos `READY` sans HLS (idempotent) → `{count, generated}`. |
| POST | `/api/admin/videos/gc?dryRun` | JWT | GC des **fichiers disque `vid-*` sans entité** (`dryRun` **défaut `true`** = recense sans supprimer) → `{orphanFilesCount, deleted}`. Ne cible que les entrées `vid-*`. |
...
| DELETE | `/api/admin/videos/{id}` | JWT | **409 + `{usedBy}`** si référencée ; sinon supprime fichiers (source/output/poster + `{id}-hls/`) + entité → 204. 404 si inconnue. |

**Schéma** : table `video` (statut + `source/output/poster_filename`, **`hls_master_filename`** (036), `duration_seconds`, `width`/`height`, `error_message`) ; colonne **`video_id`** sur `furniture`/`exhibition` (migration 034, remplace `video_url` droppée en 035) ; `video_poster` = override optionnel (poster médiathèque), `video_captions` = `.vtt`. Studio : clé `SiteContent` **`studio.video.id`** (+ `.poster`/`.captions`). **Résolution DTO** : la vidéo n'est exposée au public (`videoUrl`/`videoHls`/poster/durée résolus) **que si `READY`** ; l'état `PROCESSING`/`FAILED` et `error_message` ne fuitent jamais côté public. `SiteContentService` injecte des clés synthétiques résolues `studio.video.url`/`studio.video.hls` (lecture) quand `studio.video.id` est `READY`. Migration de l'existant : chaque ancienne vidéo → `Video(READY)` pointant sur le fichier brut (pas de re-transcodage rétro ; HLS générable ensuite via le batch).

**Rendu** : public = `<app-video-player>` (natif, réagit aux changements de source via `ngOnChanges`) ; admin = `<app-video-field>` **polle le statut** (« Traitement… » via `aria-live`, aperçu si `READY`, message `role="alert"` + **Relancer** si `FAILED`) + bouton **« Choisir une vidéo existante »** (ouvre `<app-video-picker>`).

**CSP** : `media-src 'self'`. **Upload** : limite **200 Mo** (Spring + Nginx, à garder en phase). Pas de JS inline. **Livré** : SP1 (transcodage async) · SP2 (HLS adaptatif) · SP3 (GC fichiers disque) · SP4 (médiathèque vidéo, modèle bibliothèque, picker). **Backlog** : déclenchement périodique/planifié du GC (aujourd'hui manuel via `POST /gc`).
...
- **Sliders in-preview** : édition des sliders depuis le preview en auto-save — `onSliderTitleEdit`/`onSliderZoneChange`/`onSliderDelete`/`onSliderCreate` (appels `updateSlider`/`deleteSlider`/`createSlider` + re-fetch `getPublicSliders`) ; composition via `<app-slider-composition-editor>` en overlay (`onSliderCompositionRequested` charge `getAllAdminStories` en lazy, `onSliderCompositionSave` → `replaceSliderStories`). Garde « une zone = un slider » sur le changement de zone (ne s'applique pas à `zoneKey: null`). Option **Désactivé** (`zoneKey: null`) dans le sélecteur de zone : retire le slider de l'affichage public sans le supprimer (réactivable form-side).
