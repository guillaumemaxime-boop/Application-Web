...
- `<app-home-view>` : composant standalone purement présentationnel, partagé entre la page publique et le preview admin. Inputs : `data`, `content`, `editable`. Outputs : `feedReorder`, `feedItemToggleInclude`, `textFieldEdit`, `sliderTitleEdit`, `sliderCompositionRequested`, `sliderDelete`, `sliderZoneChange`, `sliderCreate`, `storyOpen`, `viewerOpen`.
...
| **app-home-view** | Composant Angular standalone purement présentationnel, partagé entre la page publique `/` et le preview admin accueil. Inputs : `data`, `content`, `editable`. Outputs : `feedReorder`, `feedItemToggleInclude`, `textFieldEdit`, `sliderTitleEdit`, `sliderCompositionRequested`, `sliderDelete`, `sliderZoneChange`, `sliderCreate`, `storyOpen`, `viewerOpen` (les 5 outputs sliders permettent l'édition in-preview : titre, composition, suppression, zone, création). 3ème application du pattern page/view (ADR-0018). |
