...
- **`formTickSignal(form: FormGroup, destroyRef: DestroyRef): Signal<number>`** — encapsule le pattern `_formTick` : souscription `valueChanges` → incrément, désabonnement via le `DestroyRef` passé en paramètre (permet l'appel hors contexte d'injection, ex. `ngOnInit` des previews). Remplace les paires `ngOnInit`/`ngOnDestroy` manuelles de `MobilierComponent`/`ExpositionsComponent` et le même pattern dupliqué dans `FurniturePreviewComponent`/`ExhibitionPreviewComponent`.
