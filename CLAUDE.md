...
- **Pattern page/view pour fiches détail** : les composants `*-detail.component.ts` (page) délèguent le rendu visuel à `<app-*-detail-view>` (view pur, partagé public+admin). Voir ADR-0018. Pattern à respecter pour furniture-detail (fait), exhibition-detail (sous-projet 3 à venir), home (sous-projet 4 à venir).
...
- **Avant tout merge sur `main`** : systématiquement proposer à l'utilisateur de lancer un audit RGAA (accessibilité) et un audit sécurité sur la branche de la feature. L'audit sécurité passe par l'agent `security-auditor` ; l'audit RGAA passe par un agent dédié ou un check manuel ciblé (focus visibles, contrastes, labels, navigation clavier, ARIA, structure de titres). Le merge n'est proposé qu'après que les deux audits soient verts (ou que les éventuels findings soient corrigés ou explicitement acceptés par l'utilisateur).
- **Avant tout merge sur `main`** : systématiquement proposer à l'utilisateur de mettre à jour la documentation impactée par les changements de la branche : ADRs (`docs/adr/`), specs fonctionnelle et technique (`docs/SPECIFICATION_FONCTIONNELLE.md`, `docs/SPECIFICATION_TECHNIQUE.md`), README, doc de déploiement (`deploy/README.md`). Un nouveau endpoint, un changement de schema, une nouvelle convention ou une refonte architecturale = nouvelle entrée doc. Pas de merge sans confirmation explicite que la doc est à jour (ou que l'utilisateur a choisi de la mettre à jour plus tard).
